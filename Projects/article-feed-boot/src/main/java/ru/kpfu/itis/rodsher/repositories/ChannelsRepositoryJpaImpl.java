...
public class ChannelsRepositoryJpaImpl implements ChannelsRepository {
    ...
    private static final String FIND_BY_USER_AND_CHANNEL_IDS = "SELECT c FROM ChannelsToUsers c WHERE c.channel.id = :cid AND c.user.id = :uid";
    ...
    @Override
    @Transactional
    public boolean checkUserBelongingToChannel(Long userId, Long channelId) {
        try {
            entityManager.createQuery(FIND_BY_USER_AND_CHANNEL_IDS, ChannelsToUsers.class)
                    .setParameter("cid", channelId).setParameter("uid", userId).getSingleResult();
            return true;
